                           ...
                           const creal_T b)
{
    if (b.im == 0.0) {
        ...
        double brm = fabs(b.re);
        double bim = fabs(b.im);
        if (brm > bim) {
            double s = b.im / b.re;
            double d = b.re + s * b.im;
            c->im = (a.im - s * a.re) / d;
            c->re = (a.re + s * a.im) / d;
        } else if (brm == bim) {
            double half = 0.5;
            double sgnbr = b.re > 0.0 ? half : -half;
            double sgnbi = b.im > 0.0 ? half : -half;
            c->im = (a.im*sgnbr - a.re*sgnbi)/brm;
            c->re = (a.re*sgnbr + a.im*sgnbi)/brm;
        } else {
            double s = b.re / b.im;
            double d = b.im + s * b.re;
            c->im = (s * a.im - a.re) / d;
            c->re = (s * a.re + a.im) / d;
        }
    }
...
double rt_Hypot_Dbl(double a, double b)
{
    double y;
    if (a != a || b != b) {
        y = a + b;
